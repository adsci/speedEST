...
class MLModel():
    ...
    def predict(self, query: pd.DataFrame) -> float:
        ...
        if self.is_pytorch_model: # for PyTorch, convert to tensors and call the model instead
...
class FVE():
    def __init__(self, submodels, weights):
        ...
        self.res_hist_base = alt.Chart(self.residuals_base).mark_bar(opacity=0.8).encode(
                ...
                alt.Y('count()'),
                alt.Color("Model", scale=alt.Scale(domain=[m.get_name() for m in self.submodels.values()],
        ...
        self.res_PDF_base = alt.Chart(self.pdfs_base).mark_line(opacity=1).encode(x='Speed residual',y='Density',
                color=alt.Color("Model", scale=alt.Scale(domain=[m.get_name() for m in self.submodels.values()],
    ...
    def ensemble_predict(self, query: pd.DataFrame) -> tuple[pd.DataFrame, float]:
        ...
        base_preds = pd.DataFrame.from_dict(preds, orient='index', columns=['Speed [km/h]'])
    ...
    def extract_residuals(self):
        ...
        for i, df in enumerate(residuals_df):
            ...
            res_fve['Speed residual'] += self.weights[model_abbr_names[i]]*residuals_df[i]['Speed residual']
    ...
    def extract_pdfs(self):
        ...
        for i, df in enumerate(pdf_df):
            ...
            pdf_fve['Density'] += self.weights[model_abbr_names[i]]*pdf_df[i]['Density']
