...
if selected_page == 'About the project':
    ...
    st.dataframe(models.fve.get_base_est_cv().style.format("{:.3f}"))
    ...
    with tab1:
        st.altair_chart(models.fve.get_base_residual_hist(), use_container_width=True)
    with tab2:
        st.altair_chart(models.fve.get_base_residual_pdf(), use_container_width=True)
    with tab3:
        st.altair_chart(models.fve.get_residual_hist(), use_container_width=True)
    with tab4:
        st.altair_chart(models.fve.get_residual_PDF(), use_container_width=True)
    st.markdown(text.fvedesc_md[0][2], unsafe_allow_html=True)
    st.dataframe(models.fve.get_metrics().style.format("{:.3f}"))
